...
from DMDX_Django.feature_flags import (
    ENABLE_CHANNELS,
    ENABLE_WKHTMLTOPDF,
    LIGHTWEIGHT_MODE,
...
# Set DEBUG based on environment (Defaults to True for local testing)
...
INSTALLED_APPS = filter_installed_apps([
...
MIDDLEWARE = [
    ...
    'supplies.middleware.CSRFErrorMiddleware', 
...
DATABASES = {
    'default': {
        ...
        'NAME': os.getenv('DB_NAME', 'dmdx'),
        'USER': os.getenv('DB_USER', 'postgres'),
        'PASSWORD': os.getenv('DB_PASSWORD', '4646'),
...
# Throttle lazy HTMX refresh of NP tracking per order (seconds).
NP_ORDER_TRACKING_REFRESH_COOLDOWN_SECONDS = int(
    os.environ.get('NP_ORDER_TRACKING_REFRESH_COOLDOWN_SECONDS', '600')
)

# Shared cache for NP refresh cooldown
if not DEBUG:
    CACHES = {
        'default': {
